...
class AllCatViewModel(private val catApi: CatApi) : ViewModel() {
    ...
    private val pagingConfig = PagingConfig(enablePlaceholders = false, pageSize = 30)

    private val pager = Pager(pagingConfig, initialKey = 0) {
        object : PagingSource<Int, Cat>() {

            override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Cat> {
                val page = params.key ?: 0
                return try {
                    val cats = catApi.getAll(page = page, limit = pagingConfig.pageSize)
